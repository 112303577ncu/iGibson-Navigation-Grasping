...
# 照片存放資料夾
SAVE_DIR = r"C:\Users\user\Downloads\rear_cam_dataset"
...
def save_frame(frame):
    now = datetime.now()
    filename = now.strftime("rear_%Y%m%d_%H%M%S_%f")[:-3] + ".jpg"
...
def main():
    ...
    if not cap.isOpened():
        ...
        print("[CHECK] 確認 Jetson IP、camera stream、網路連線是否正常")
        try:
    ...
    print("[INFO] Start capturing rear cam images.")
    print(f"[INFO] Save directory: {SAVE_DIR}")
    print(f"[INFO] Capture interval: {CAPTURE_INTERVAL} sec")
    print("==================================================")
    print("W: forward")
    print("S: backward")
    print("A: turn_left")
    ...
    try:
        while True:
            ...
            # 每 1 秒自動存一張
            if auto_capture_enabled and now - last_capture_time >= CAPTURE_INTERVAL:
                if save_frame(frame):
                    capture_count += 1
            ...
            status_text = "ON" if auto_capture_enabled else "PAUSED"

            cv2.putText(
                display,
                f"Auto capture: {status_text} | every {CAPTURE_INTERVAL:.1f}s",
                (15, 30),
            ...
            cv2.putText(
                display,
                "K save now | P pause auto | ESC quit",
                (15, 165),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.55,
                (255, 255, 255),
            ...
            cv2.imshow("Rear Cam Auto Capture + Keyboard Control", display)
            ...
            if key == 27:
                ...
                print(f"[KEY] auto_capture_enabled = {auto_capture_enabled}")
