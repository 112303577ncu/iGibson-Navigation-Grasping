...
class DeployConfig:
    ...
    # Repo integration: this stack lives at grasp/v21/ and reuses the URDF + meshes
    ...
    # Two tiers, because the evidence behind them is different.
    #
    # trained_*: the region the formal evaluation covered -- x(0.20, 0.33),
    #   y(+-0.10), 97/100 on a held-out seed (see the acceptance note above
    #   home_deg). Outside it the policy was never measured at all, so a target
    #   there is not "probably fine", it is unevaluated.
    # documented_*: the narrower range manifest.json's how_to_run calls valid,
    #   x 0.20-0.28, y -0.09-0.08. Between the two the policy has been evaluated
    #   but the package does not vouch for it.
    #
    # This matters specifically because of vision. Hand-typed --obj-x stays where
    # you put it, but the arm camera at the C3 pose sees ground from about
    # x=0.200 to x=0.318 (integration/arm_cam_geometry.py), which reaches the very
    # edge of the trained region at the near end -- the bottom row of the frame is
    # already 0.2 mm short of it, so a small calibration error puts real detections
    # outside. A detector that is working perfectly can therefore hand the policy a
    # target it was never trained for, and nothing downstream would notice: the
    # coordinate is finite, in frame, and geometrically sound.
    trained_x_range: Tuple[float, float] = (0.20, 0.33)
    trained_y_range: Tuple[float, float] = (-0.10, 0.10)
    documented_x_range: Tuple[float, float] = (0.20, 0.28)
    documented_y_range: Tuple[float, float] = (-0.09, 0.08)
...
class DetectionReceiver:
    ...
    Expected JSON format:
        {"x": 0.20, "y": 0.00, "z": 0.02, "height": 0.013,
         "cam_pose": [90.0, 67.08, 9.79, 9.79, 90.0, 30.0],
         "cam_pose_name": "v21_c3_grasp_home"}
