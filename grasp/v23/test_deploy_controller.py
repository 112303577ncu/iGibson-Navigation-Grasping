...
def test_small_home_residual_has_a_bounded_recovery_window():
    ...
    # S2 is stuck 2.1 deg from the grasp home. This reproduces the hardware report
    # without pretending that a large or unknown arm offset is safe for camera
    # geometry.
    #
    # Derived from DeployConfig.home_deg rather than written out, because the
    # subject here is the 2.1 deg RESIDUAL, not any particular pose. The v21 copy
    # of this test hard-coded C3's [90, 64.98, 9.79, 9.79, 90, 30]; carried to v23
    # unchanged that becomes a 9.22 deg residual against E1 and the test fails for
    # a reason that has nothing to do with what it is checking.
    with contextlib.redirect_stdout(io.StringIO()):
        import x3plus_real_grasp as _X
    stalled = list(_X.DeployConfig().home_deg)
    stalled[1] -= 2.1
    plant = FakeServoPlant(stalled, max_delta_deg=0.0)
    ctrl, cfg = build(plant)
